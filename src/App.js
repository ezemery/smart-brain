...
class App extends Component {
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input});
      fetch('http://161e8afd.ngrok.io/imageurl', {
        method: 'post',
        headers: {
          'Content-Type': 'application/json',
      ...
      .then(response => {
        if (response) {
          fetch('http://161e8afd.ngrok.io/image', {
            method: 'put',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': window.localStorage.getItem("token")
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        {isProfileOpen && 
        <Modal>
            <Profile  loadUser={this.loadUser} user={this.state.user} isProfileOpen={isProfileOpen} toggleModal={this.toggleModal}/>
        </Modal>}
        { route === 'home'
          ? <div>
              <Logo />
